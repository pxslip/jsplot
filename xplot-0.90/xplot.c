...
static void compute_window_coords(struct plotter *pl, command *com)
{
  ...
  if (com->type == TITLE || com->type == XLABEL || com->type == YLABEL)
  {
...
#define ycmp(ya, yb, op) ccmp(pl->y_type, ya, yb, op)
  ...
  if (xcmp(com->xa, pl_x_left, <))
    if (ycmp(com->ya, pl_y_top, >))
      loc1 = 0;
    else if (ycmp(com->ya, pl_y_bottom, <))
      loc1 = 6;
    else
      loc1 = 3;
  else if (xcmp(com->xa, pl_x_right, >))
    if (ycmp(com->ya, pl_y_top, >))
      loc1 = 2;
    else if (ycmp(com->ya, pl_y_bottom, <))
      loc1 = 8;
  ...
  case LINE:
    if (xcmp(com->xb, pl_x_left, <))
      if (ycmp(com->yb, pl_y_top, >))
        loc2 = 0;
      else if (ycmp(com->yb, pl_y_bottom, <))
        loc2 = 6;
      else
        loc2 = 3;
    else if (xcmp(com->xb, pl_x_right, >))
      if (ycmp(com->yb, pl_y_top, >))
        loc2 = 2;
      else if (ycmp(com->yb, pl_y_bottom, <))
        loc2 = 8;
...
void display_plotter(PLOTTER pl)
{
...

  {
    ...
    if (geom)
    {
      ...
      {
        if (flags & XNegative)
          x += DisplayWidth(pl->dpy, ScreenNumberOfScreen(pl->screen)) - width - borderw * 2;
        else
          x += borderw;
      }
      if (flags & YValue)
      {
        if (flags & YNegative)
          y += DisplayHeight(pl->dpy, ScreenNumberOfScreen(pl->screen)) - height - borderw * 2;
        else
          y += borderw;
      }
    }
...
int main(int argc, char *argv[])
{
  ...
  for (; i < argc && *argv[i] == '-'; i++)
  {
    ...
    else if (strcmp("-d", argv[i]) == 0 || strcmp("-display", argv[i]) == 0)
    {
      i++;
      if (dpy == 0)
      {
  ...
  if (the_plotter_list)
  {
#define BUTFIRSTPLOTTERS       \
  pl = the_plotter_list->next; \
  pl != NULL;                  \
  pl = pl->next
    if (x_synch)
    {
...
#define ALLPLOTTERS      \
  ...
  for (ALLPLOTTERS)
  {
    if (option_one_at_a_time == FALSE || pl->next == 0)
    {
      display_plotter(pl);
    }
  }
  ...
  do
  {
    ...
    lXPoint a, b;
    if (XPending(dpy) == 0 && (dpy2 == 0 || XPending(dpy2) == 0))
    {
      int visible_count = 0;
      for (ALLPLOTTERS)
      {
        ...
        if (visible_count == 0 && pl->next == 0 && pl->win == 0 && option_one_at_a_time)
          display_plotter(pl);
          ...
          for (c = pl->commands; c != NULL; c = c->next)
            if (c->mapped)
              if (c->needs_redraw)
              {
                ...
                if (c->decoration || c->type == TITLE || c->type == XLABEL || c->type == YLABEL)
                  gc = pl->decgc;
                else if (c->color >= 0 && c->color < NColors)
                  gc = pl->gcs[c->color];
                else
                  gc = pl->gcs[0];
...
#undef D
                ...
                case YLABEL:
                {
                  ...
                  case TO_THE_RIGHT:
                    ...
                    break;
                  default:
                    panic("drawloop, case TEXT: unknown text positioning");
                  }
                  switch (c->position)
                  {
                  case CENTERED:
                  ...
                  case TO_THE_RIGHT:
                    ...
                    break;
                  case ABOVE:
                    p.y -= space + font_descent;
                    break;
                  case BELOW:
                    p.y += space + font_ascent;
                    break;
                  default:
                    panic("drawloop, case TEXT: unknown text positioning");
                  }
                ...
                if (XEventsQueued(dpy, QueuedAlready) != 0)
                  break;
                if (dpy2 != 0 &&
                    XEventsQueued(dpy2, QueuedAlready) != 0)
                  break;
              }
          if (c == NULL)
            pl->clean = 1;
/* #define SAVE_PRINTOUTS */
    ...
    for (ALLPLOTTERS)
      if (pl->dpy == dpy_of_event && pl->win == event.xany.window)
        break;
    if (pl == 0)
      ...
      case NORMAL:
        ...
        case Button1:
          if (event.xbutton.state & ShiftMask)
            pl->state = PRINTING;
          else if (event.xbutton.y > pl->size.y + pl->origin.y)
            pl->state = HZOOM;
    ...
    case ButtonRelease:
      ...
      case VZOOM:
      {
            ...
            (abs(pl->raw_dragstart.y - event.xbutton.y) > 7))
        {
          ...
          if (((pl->state != VZOOM) && (abs(dragstart.x - dragend.x) > 7)) || ((pl->state != HZOOM) && (abs(dragstart.y - dragend.y) > 7)))
          {

            int newviewno;

            newviewno = pl->viewno + 1;
            if (newviewno == NUMVIEWS)
              newviewno -= 1;
        ...
        else
        {
          if (pl->viewno > 1)
          {
            ...
            if ((x_synch && (xcmp(pl_x_left, pl->x_left[pl->viewno - 1], !=) || xcmp(pl_x_right, pl->x_right[pl->viewno - 1], !=))) || (y_synch && (ycmp(pl_y_top, pl->y_top[pl->viewno - 1], !=) || ycmp(pl_y_bottom, pl->y_bottom[pl->viewno - 1], !=))))
...
#ifdef __GNUC__
static inline int mystrcmp(char *s1, char *s2)
{
  while (*s1 == *s2++)
    if (*s1++ == '\0')
      return 0;
...
#define parseerror(s)                                              \
  ...
  do
  {
    ...
    tokens = gettokens(fp);
    if (tokens == 0)
      parseerror("EOF before first line of input");
...
#define COLORfromTOK3 (com->color = ntokens == 4 ? parse_color(tokens[3]) : pl->current_color)
    ...
    {
      if (not_ntokens_equal_to_3_or_4)
        parseerror("input format error");
      com = new_command(pl);
      com->type = X;
    ...
    else if (mystrcmp(tokens[0], ".") == 0 || mystrcmp(tokens[0], "dot") == 0)
    {
      if (not_ntokens_equal_to_3_or_4)
        parseerror("input format error");
      com = new_command(pl);
...
notintex {
      1.5 inch 5.0 inch translate } if
", fp);
  }
...
#define ytoPSy(yyyy) ((int)rint(pspl.size.y - yyyy))
  ...
  for (c = pspl.commands; c != NULL; 
    ...
    if (finished_decoration == TRUE && (c->decoration || c->type == TITLE || c->type == XLABEL || c->type == YLABEL))
      continue;
    if (finished_decoration == FALSE && !(c->decoration || c->type == TITLE || c->type == XLABEL || c->type == YLABEL))
      continue;
